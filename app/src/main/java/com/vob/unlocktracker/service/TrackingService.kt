...
import com.vob.unlocktracker.util.Constants.FASTEST_LOCATION_INTERVAL
import com.vob.unlocktracker.util.Constants.LOCATION_UPDATE_INTERVAL
...
import com.vob.unlocktracker.util.Constants.TIMER_UPDATE_INTERVAL
...
class TrackingService: LifecycleService() {
    ...
    private fun startTimer() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            {
                //time difference between now and when it started
                lapTime = System.currentTimeMillis() - timeStarted

                timeRunInMillis.postValue(timeRun + lapTime) //total time run till now

                if (timeRunInMillis.value!! >= lastSecondTimeStamp + 1000L)
                {
                    //to measure 1 second has passed
    ...
    private fun updateNotificationTrackingState(isTracking: Boolean) {
        ...
        //to remove all actions already present in our notification
    ...
    private fun addPathPoint(location: Location?) {
        location?.let {
            ...
            pathPoints.value?.apply {
                last().add(pos) //to add at end of the list
